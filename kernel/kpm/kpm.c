...
noinline NO_OPTIMIZE void kpm_load_module_path(const char *path,
                                                      const char *args,
                                                      void *ptr, int *result)
{
    pr_info("kpm: Stub function called (kpm_load_module_path). "
            "path=%s args=%s ptr=%p\n",
            path, args, ptr);

    __asm__ volatile("nop");
}
EXPORT_SYMBOL(kpm_load_module_path);

noinline NO_OPTIMIZE void kpm_unload_module(const char *name, void *ptr,
                                                   int *result)
{
    pr_info("kpm: Stub function called (kpm_unload_module). "
            "name=%s ptr=%p\n",
            name, ptr);

    __asm__ volatile("nop");
}
EXPORT_SYMBOL(kpm_unload_module);

noinline NO_OPTIMIZE void kpm_num(int *result)
{
    pr_info("kpm: Stub function called (kpm_num).\n");

    __asm__ volatile("nop");
}
EXPORT_SYMBOL(kpm_num);

noinline NO_OPTIMIZE void kpm_info(const char *name, char *buf,
                                          int bufferSize, int *size)
{
    pr_info("kpm: Stub function called (kpm_info). "
            "name=%s buffer=%p\n",
            name, buf);
...
noinline NO_OPTIMIZE void kpm_list(void *out, int bufferSize,
                                          int *result)
{
    pr_info("kpm: Stub function called (kpm_list). "
            "buffer=%p size=%d\n",
            out, bufferSize);
}
EXPORT_SYMBOL(kpm_list);

noinline NO_OPTIMIZE void kpm_control(const char *name, const char *args,
                                             long arg_len, int *result)
{
    pr_info("kpm: Stub function called (kpm_control). "
            "name=%p args=%p arg_len=%ld\n",
            name, args, arg_len);

    __asm__ volatile("nop");
}
EXPORT_SYMBOL(kpm_control);

noinline NO_OPTIMIZE void kpm_version(char *buf, int bufferSize)
{
    pr_info("kpm: Stub function called (kpm_version). "
            "buffer=%p\n",
            buf);
}
EXPORT_SYMBOL(kpm_version);

noinline int handle_kpm(unsigned long control_code, unsigned long arg1,
                               unsigned long arg2, unsigned long result_code)
{
    int res = -1;
    if (control_code == KPM_LOAD) {
        ...
        kpm_load_module_path((const char *)&kernel_load_path,
                                    (const char *)&kernel_args_buffer, NULL,
                                    &res);
    } else if (control_code == KPM_UNLOAD) {
        ...
        kpm_unload_module((const char *)&kernel_name_buffer, NULL, &res);
    } else if (control_code == KPM_NUM) {
        kpm_num(&res);
    } else if (control_code == KPM_INFO) {
        char kernel_name_buffer[256];
        char buf[256];
        int size;
        ...
        kpm_info((const char *)&kernel_name_buffer, (char *)&buf,
                        sizeof(buf), &size);
        ...
        kpm_list((char *)&buf, sizeof(buf), &res);
    ...
    } else if (control_code == KPM_CONTROL) {
        ...
        kpm_control((const char *)&kpm_name, (const char *)&kpm_args,
                           arg_len, &res);

    } else if (control_code == KPM_VERSION) {
        char buffer[256] = { 0 };

        kpm_version((char *)&buffer, sizeof(buffer));
...
int do_kpm(void __user *arg)
{
    ...
    return handle_kpm(cmd.control_code, cmd.arg1, cmd.arg2,
                             cmd.result_code);
}
